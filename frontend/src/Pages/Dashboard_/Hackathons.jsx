...
const Hackathons = () => {
  ...
  const getFilteredHackathons = () => {
    ...
    // Make sure hackathonData is an array before trying to filter it
  ...
  // Get filtered hackathons once and handle empty arrays safely
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-light-primary text-dark-primary font-outfit">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredHackathons.map((hackathon, index) => (
          <motion.div
            ...
            className="bg-light-secondary2 rounded-xl shadow-lg overflow-hidden"
            whileHover="hover"
          >
            <div className="p-6">
              <h3 className="text-xl font-bold mb-3 text-dark-primary font-poppins">{hackathon.name}</h3>
              
              <div className="flex flex-wrap gap-2 mb-4">
                {hackathon.domains && hackathon.domains.map((domain) => (
                  <span key={domain} className="bg-dark-secondary1 text-light-secondary2 text-xs px-3 py-1 rounded-full font-dmsans">
                    {domain}
                  </span>
                ))}
              </div>
              ...
              <Link
                ...
                className="block w-full bg-dark-primary text-light-secondary2 px-4 py-2 rounded-lg text-center font-poppins hover:bg-dark-secondary1 transition-colors duration-300"
              >
                View Details
              </Link>
            </div>
      ...
      {filteredHackathons.length === 0 && (
        ...
        >
          <svg className="mx-auto h-12 w-12 text-dark-secondary1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
